...
def format_help() -> str:
    return (
        ...
        "В текущей версии платёж считается закрытым для планирования, когда нужная сумма полностью зарезервирована ботом. "
        "Отдельно отмечать оплату не нужно.\n\n"
        ...
        "Некоторые расширенные функции временно скрыты, пока мы стабилизируем основной расчёт доходов, платежей и резервов.\n\n"
...
def format_reminder(data) -> str:
    ...
    if days == 0:
        return f"Сегодня платёж: {title} — {amount}.\nПроверь, что нужная сумма зарезервирована в плане."
    if days == 1:
